...
from utils import AddRandomNoisePE
...
def add_pe_to_dataset(dataset, pe_dim, walk_length=20, attr_name='pe'):
    ...
    """
    from torch_geometric.data import Data
    RNPE = AddRandomNoisePE(walk_length, attr_name=attr_name)
            ...
            try:
                ...
                for key, value in data.__dict__.items():
                    if not key.startswith('_'):
                        data_obj[key] = value
                new_data = RNPE.forward(data_obj)
...
def get_model_save_path(dataset_name, layer_type, hidden_dim, num_layers, pos_encodings):
    ...
    pe_suffix = "_rn_pe" if pos_encodings else ""
...
def evaluate_main_fixed_train_noise_pes(args,
                           t_feature=0,
                           t_structure=0,
                           eval_on_val=False):
    """
